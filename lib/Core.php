...
class PeoplePod {
	...
	public $PODS;
	// contains info about installable pods
	...
	// 1 = debug messages 2 = sql commands
	...
	function PeoplePod($PARAMETERS = null) {
		...
		if (!file_exists("$libPath/etc/options.php") || !
		include ("$libPath/etc/options.php")) {
	...
	function enablePOD($name) {
		...
		$POD -> setLibOptions('enable_' . $name, 'true');
		// tell the library this is turned on
	...
	function saveLibOptions($force = false) {
		...
		$fh = fopen($file, 'w');
		if (!$fh) { $this -> throwError("Can't open config file!  Check file permissions on " . $this -> libOptions('etcPath') . "/options.php");
			return false;
		}
		fwrite($fh, "<?\n");
