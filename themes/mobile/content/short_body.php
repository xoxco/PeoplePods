...
?>
		<article class="attributed_content content_body">
				<header>
					...
					<h1><a href="<?php $doc->write('permalink'); ?>" title="<?php $doc->write('headline'); ?>" rel="external"><?php $doc->write('headline'); ?></a></h1>
				</header>
				...
				<?php if ($img = $doc->files()->contains('file_name','img')) { ?>
					<p class="content_image"><a href="<?php $doc->write('permalink'); ?>"><img src="<?php $img->write('thumbnail') ?>" border="0" /></a></p>
				<?php } ?>			
				...
				<ul class="content_options">
					<li class="comments_option">
						<a href="<?php $doc->write('permalink'); ?>" rel="external"><?php  if ($doc->comments()->totalCount() > 0) {  echo $doc->comments()->totalCount() . " comments"; } else { echo "No comments"; } ?></a>
					</li>
					<?php if ($doc->POD->isAuthenticated()) { ?>
						<li class="watching_option">
							<a href="#toggleFlag" data-ajax="false" data-flag="watching" data-active="Stop tracking" title="Track new comments on the dashboard" data-inactive="Track" data-content="<?php echo $doc->id; ?>" class="trackingLink <?php if ($doc->hasFlag('watching',$POD->currentUser())) {?>active<?php } ?>">Track</a>
						</li>
					<?php } ?>				
					...
					<?php if ($doc->isEditable()) { ?>
						<li class="delete_option">
							<a href="#deleteContent" data-ajax="false" data-content="<?php echo $doc->id; ?>">Delete</a>
						</li>
					<?php } ?>
				</ul>
		</article>
